...
describe("Curve", () => {
  ...
  test("sharedKey", () => {
    ...
    expect(sharedKey1).toBeInstanceOf(Buffer);
    expect(sharedKey2).toBeInstanceOf(Buffer);
...
describe("signedKeyPair", () => {
  test("generates signed key pair", () => {
    ...
    const isValid = Curve.verify(identityKeyPair.public, pubKey, result.signature);
    expect(isValid).toBe(true);
  });
});
