...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const sendPeerDataOperationMessage = async (
		pdoMessage: proto.MessagePeerDataOperationRequestMessage,
	): Promise<string> => {
		...
		const protocolMessage: proto.Message = {
			protocolMessage: {
				peerDataOperationRequestMessage: pdoMessage,
				type: proto.MessageProtocolMessageType
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroup || isStatus) {
				...
				if (senderKeyJids.length) {
					...
					const senderKeyMsg: proto.Message = {
	...
	const getMediaType = (message: proto.Message) => {
	...
	return {
		...
		getUSyncDevices,
		updateMediaMessage: async (message: proto.WebMessageInfo) => {
					...
					if (result) {
						if (result.error) {
							...
							try {
								...
								if (
									media.result !==
									proto.MediaRetryNotificationResultType.SUCCESS
								) {
									const resultStr =
										proto.MediaRetryNotificationResultType[media.result!];
									throw new Boom(
