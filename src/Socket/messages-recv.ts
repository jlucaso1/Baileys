...
import { uint8ArrayToBase64, utf8StringToUint8Array } from "../Utils/buffer";
...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const processNotification = async (node: BinaryNode) => {
		...
		switch (nodeType) {
			...
			case "link_code_companion_reg": {
				...
				const encryptPayload = new Uint8Array([
					...authState.creds.signedIdentityKey.public,
				...
				const identityPayload = new Uint8Array([
					...companionSharedKey,
					...identitySharedKey,
					...random,
				]);
				authState.creds.advSecretKey = uint8ArrayToBase64(
					hkdf(identityPayload, 32, {
						info: "adv_secret",
					}),
				);
				await query({
	...
	async function decipherLinkPublicKey(data: Uint8Array) {
	...
	function toRequiredBuffer(data: Uint8Array | undefined) {
		if (data === undefined) {
			...
			await Promise.all([
				processingMutex.mutex(async () => {
					...
					if (
						...
						(status > proto.WebMessageInfoStatus.DELIVERY_ACK || !isNodeFromMe)
					) {
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		if (
			msg.message?.protocolMessage?.type ===
				proto.MessageProtocolMessageType.SHARE_PHONE_NUMBER &&
	...
	const fetchMessageHistory = async (
		...
		const pdoMessage: proto.MessagePeerDataOperationRequestMessage = {
			...
			peerDataOperationRequestType:
				proto.MessagePeerDataOperationRequestType.HISTORY_SYNC_ON_DEMAND,
		};
	...
	const requestPlaceholderResend = async (
		...
		const pdoMessage = {
			...
			peerDataOperationRequestType:
				proto.MessagePeerDataOperationRequestType.PLACEHOLDER_MESSAGE_RESEND,
		};
	...
	ev.on("call", ([call]) => {
		...
		if (
			...
			if (call.status === "timeout") {
				...
				msg.message = { call: { callKey: utf8StringToUint8Array(call.id) } };
			}
