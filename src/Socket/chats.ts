...
import { writeBinaryNode } from "../Utils/proto-utils";
import {
	type BinaryNode,
	getBinaryNodeChild,
...
import { Label, type LabelActionBody } from "../Types/Label";
...
export const makeChatsSocket = (config: SocketConfig) => {
			...
			content: [
				{
					tag: "status",
					attrs: {},
					content: utf8StringToUint8Array(status),
				},
			],
		});
	};
	...
	const getBusinessProfile = async (
		jid: string,
	): Promise<WABusinessProfile | undefined> => {
	 ...
	 */
	const presenceSubscribe = (toJid: string, tcToken?: Uint8Array) =>
		sendNode({
			tag: "presence",
			attrs: {
				to: toJid,
