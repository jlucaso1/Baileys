...
const assertColor = async (color) => {
	...
	if (typeof color === "number") {
		...
		assertedColor = Number.parseInt(hex, 16);
...
export const prepareWAMessageMedia = async (
	...
	if (cacheableKey) {
		...
		options.mediaCache?.set(
			cacheableKey,
			writeBinaryNode(proto.writeMessage, obj),
		);
	}
...
export const generateWAMessageFromContent = (
	...
	if (quoted) {
		...
		if (
			...
			quotedContent.contextInfo = undefined;
		}
...
export const extractMessageContent = (
	...
	const extractFromTemplateMessage = (
		msg:
			| proto.MessageTemplateMessageHydratedFourRowTemplate
			| proto.MessageButtonsMessage,
	) => {
		if (msg.imageMessage) {
...
export const assertMediaContent = (
	content: proto.Message | null | undefined,
) => {
