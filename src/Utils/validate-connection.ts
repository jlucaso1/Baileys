...
const getUserAgent = (config: SocketConfig): proto.ClientPayloadUserAgent => {
	...
	const platform = config.mobile
		? proto.ClientPayloadUserAgentPlatform.IOS
		: proto.ClientPayloadUserAgentPlatform.WEB;
...
const getPlatformType = (platform: string): proto.DevicePropsPlatformType => {
	...
	return (
		proto.DevicePropsPlatformType[platformType] ||
		proto.DevicePropsPlatformType.DESKTOP
	);
};
...
export const configureSuccessfulPairing = (
	...
	const { details, hmac } = readBinaryNode(
		proto.readADVSignedDeviceIdentityHMAC,
		deviceIdentityNode.content as Buffer,
	);

	if (!details || !hmac) {
		throw new Boom("Missing details or hmac in device identity", {
			data: stanza,
		});
	}
	...
	if (!accountSignatureKey || !accountSignature || !deviceDetails) {
		throw new Boom(
			"Missing accountSignatureKey, accountSignature or deviceDetails in account",
			{ data: stanza },
		);
	}
	...
	const deviceIdentity = readBinaryNode(
		proto.readADVDeviceIdentity,
		account.details!,
	);
