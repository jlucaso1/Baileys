...
export function getMediaKeys(
	...
	if (typeof buffer === "string") {
		buffer = utf8StringToUint8Array(buffer.replace("data:;base64,", ""));
	}
...
export const extractImageThumb = async (
	bufferOrFilePath: Readable | Uint8Array | string,
	width = 32,
) => {
	if (bufferOrFilePath instanceof Readable) {
		bufferOrFilePath = await readableToBuffer(bufferOrFilePath);
	}
...
export const generateProfilePicture = async (mediaUpload: WAMediaUpload) => {
	let bufferOrFilePath: Uint8Array | string;
	if (isUint8Array(mediaUpload)) {
		bufferOrFilePath = mediaUpload;
	} else if ("url" in mediaUpload) {
		bufferOrFilePath = mediaUpload.url.toString();
	} else {
		bufferOrFilePath = await readableToBuffer(mediaUpload.stream);
	}

	const lib = await getImageProcessingLibrary();
	let img: Promise<Uint8Array>;
	if ("sharp" in lib && typeof lib.sharp?.default === "function") {
		img = lib.sharp
...
export const mediaMessageSHA256B64 = (message: WAMessageContent) => {
	...
	return media?.fileSha256 && uint8ArrayToBase64(media.fileSha256);
};

export async function getAudioDuration(buffer: Uint8Array | string | Readable) {
...
export async function getAudioWaveform(
	...
	try {
		const audioDecode = (buffer: ArrayBuffer | Uint8Array) =>
		...
		if (isUint8Array(buffer)) {
			...
			const rStream = createReadStream(buffer);
			audioData = await readableToBuffer(rStream);
		} else {
			audioData = await readableToBuffer(buffer);
		}
...
export const toReadable = (buffer: Uint8Array) => {
...
export const getStream = async (
	...
	if (isUint8Array(item)) {
		return { stream: bufferToReadable(item), type: "buffer" } as const;
	}
...
export const encryptedStream = async (
	...
	function onChunk(buff: Uint8Array) {
...
export const downloadEncryptedContent = async (
	...
	const pushBytes = (bytes: Uint8Array, push: (bytes: Uint8Array) => void) => {
	...
	const output = new Transform({
		transform(chunk, _, callback) {
			let data = new Uint8Array(remainingBytes.length + chunk.length);
			data.set(remainingBytes);
			data.set(chunk, remainingBytes.length);
