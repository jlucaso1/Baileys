...
type FetchAppStateSyncKey = (
	keyId: string,
) => Promise<proto.MessageAppStateSyncKeyData | null | undefined>;
...
const generateMac = (
	operation: proto.SyncdMutationSyncdOperation,
	data: Uint8Array,
	keyId: Uint8Array,
	key: Uint8Array,
) => {
...
const to64BitNetworkOrder = (e: number): Uint8Array => {
	const buffer = new Uint8Array(8); // 8 bytes for 64-bit
	const view = new DataView(buffer.buffer);
	view.setUint32(4, e, false); // Write the 32-bit number at offset 4 in big-endian order
	return buffer;
};
...
const generatePatchMac = (
	...
	// Convert type (string) to Uint8Array (UTF-8)
	const typeArray = new TextEncoder().encode(type);

	// Concatenate all arrays
	const total = concatUint8Arrays(
		snapshotMac,
		...valueMacs,
		to64BitNetworkOrder(version),
		typeArray,
...
export const encodeSyncdPatch = async (
	...
	const base64Index = uint8ArrayToBase64(indexMac);
	...
	for (const msgMutation of msgMutations) {
		...
		const operation =
			"operation" in msgMutation
				...
				: proto.SyncdMutationSyncdOperation.SET;
		const record =
			"record" in msgMutation && !!msgMutation.record
				? msgMutation.record
				: (msgMutation as proto.SyncdRecord);
	...
	await Promise.all(
		collectionNodes.map(async (collectionNode) => {
			...
			if (snapshotNode && !!snapshotNode.content) {
				...
				const blobRef = readBinaryNode(
					proto.readExternalBlobReference,
...
export const processSyncAction = (
	...
	function getChatUpdateConditional(
		id: string,
		msgRange: proto.SyncActionValueSyncActionMessageRange | null | undefined,
